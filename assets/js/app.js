...
function openPrintWin(title, contentHtml){
  ...
  const docHtml = `<!DOCTYPE html><html lang="ko"><head><meta charset="utf-8"><title>${title}</title>
    ...
    <style>
      ...
      .print-root{display:block; width:100%; min-height:10px}
      @media print{
        html,body{background:#fff !important; overflow:visible !important;}
        body{padding:0 !important; margin:0 !important;}
        body *{visibility:hidden !important;}
        .print-root, .print-root *{visibility:visible !important;}
        .print-root{position:static !important; inset:auto !important; left:auto !important; top:auto !important; width:auto !important; height:auto !important; display:block !important; overflow:visible !important;}
        .print-root .page-break{page-break-after:always;}
...
(function(){
  ...
  window.loadStfRecord = function() {
    ...
    const rows = [];
    (db.mat || []).forEach(m => {
      if (String(m.stfId) !== String(stfId)) return;
      (m.logs || []).forEach(l => {
        try { if (typeof ensureLogFields === 'function') ensureLogFields(l, m); } catch(e){}
        if (!(l.date || '').startsWith(ym)) return;
  ...
  window.openAddRecModal = function() {
    ...
    if (!stfId) { toast('지원단을 먼저 선택하세요', 'warning'); return; }
    try { if (typeof buildIndex === 'function') buildIndex(); } catch(e){}
    const mats = (db.mat || []).filter(m => String(m.stfId) === String(stfId) && m.st === 'active');
    if (!mats.length) {
      toast('선택한 지원단의 활성 매칭이 없습니다. 먼저 매칭을 확인하세요.', 'warning');
      return;
    }
  ...
  window.saveAddRec = async function(btn) {
    ...
    const m = (db.mat || []).find(x => String(x.id) === String(matId));
  ...
  window.goTab = function(id, btn) {
    ...
    if (id === 't5') {
      setTimeout(function() {
        ...
        const vm = document.getElementById('ver-month');
        if (vm && !vm.value) vm.value = typeof thisMonth === 'function' ? thisMonth() : new Date().toISOString().slice(0,7);
        const activeSub = document.querySelector('#t5 .subtab-content.active');
        if (activeSub && activeSub.id === 'sub-t5-record') safeCall(window.loadStfRecord);
        else safeCall(window.loadVerify);
      }, 50);
    }
  };
})();
