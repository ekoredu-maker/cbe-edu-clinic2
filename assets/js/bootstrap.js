...
subscribe(({ event }) => {
  if (event.startsWith('persist:')) console.debug('[V12.4 store event]', event);
});
...
window.addEventListener('load', () => {
  ...
  if (badge) {
    ...
    badge.textContent = text.replace(/V\d+(?:\.\d+)?[^•]*/,'V12.4 Term-Based Edition');
  }
  document.title = (document.title || '학습클리닉 통합관리').replace(/V\d+(?:\.\d+)?/,'V12.4');
});
